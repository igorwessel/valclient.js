...
const group = new Group(fetch as Fetch, post as Post, del as Delete, currentUserId);
...
beforeEach(() => {
    fetch.mockResolvedValueOnce(mockedCurrentGroupUser);
});
...
test("should return current group from user authenticated", async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(`/parties/v1/players/${currentUserId}`, "glz");

    expect(data).toEqual(mockedCurrentGroupUser);
});

test("should return details current group from user authenticated", async () => {
    fetch.mockResolvedValueOnce(mockedCurrentGroupDetails);

    const data = await group.currentDetails();

    expect(fetch).toHaveBeenCalledTimes(2);

    expect(fetch).toHaveBeenNthCalledWith(1, `/parties/v1/players/${currentUserId}`, "glz");

    expect(fetch).toHaveBeenNthCalledWith(2, `/parties/v1/parties/${currentGroupID}`, "glz");
...
test("should remove a player current group from user authenticated", async () => {
    ...
    expect(del).toHaveBeenCalledWith(`/parties/v1/players/${playerRemoved}`, "glz");
...
test("should remove user authenticated from current group", async () => {
    ...
    expect(del).toHaveBeenCalledWith(`/parties/v1/players/${currentUserId}`, "glz");

    expect(data).toBeTruthy();
});

test("should sets whether a party member is ready for queueing", async () => {
    post.mockResolvedValueOnce(mockedCurrentGroupDetails);
...
test("should refresh competitive tier from user authenticated", async () => {
    post.mockResolvedValueOnce(true);
...
test("should change queue in current group from user authenticated", async () => {
    ...
    post.mockResolvedValueOnce(groupChanged);
    ...
    expect(post).toHaveBeenCalledWith(`/parties/v1/parties/${currentGroupID}/queue`, "glz", { queueID: queue });
...
test("should start a custom game from user authenticated", async () => {
    ...
    expect(post).toHaveBeenCalledWith(`/parties/v1/parties/${currentGroupID}/startcustomgame`, "glz");
...
test("should enter in matchmaking queue from user authenticated", async () => {
    ...
    expect(post).toHaveBeenCalledWith(`/parties/v1/parties/${currentGroupID}/matchmaking/join`, "glz");
...
test("should leave in matchmaking queue from user authenticated", async () => {
    ...
    expect(post).toHaveBeenCalledWith(`/parties/v1/parties/${currentGroupID}/matchmaking/leave`, "glz");
...
test("should change to close acessibility state current group from user authenticated", async () => {
    ...
    post.mockResolvedValueOnce(groupState);
    ...
    expect(post).toHaveBeenCalledWith(`/parties/v1/parties/${currentGroupID}/accessibility`, "glz", {
        accessibility: "CLOSED",
    });
...
test("should set custom game settings from user autenticated", async () => {
    ...
    post.mockResolvedValueOnce(customGame);
    ...
    expect(post).toHaveBeenCalledWith(`/parties/v1/parties/${currentGroupID}/customgamesettings`, "glz", {
        Map: "/Game/Maps/Ascent/Ascent",
        Mode: "/Game/GameModes/Bomb/BombGameMode.BombGameMode_C",
        GamePod: "aresriot.aws-rclusterprod-sae1-1.br-gp-saopaulo-1",
        GameRules: {
...
test("should invite someone by display name", async () => {
    ...
    post.mockResolvedValueOnce(groupInvite);
    ...
    expect(post).toHaveBeenCalledWith(`/parties/v1/parties/${currentGroupID}/invites/name/${name}/tag/${tag}`, "glz", {
        name: "iws",
        tag: "777",
    });
...
test("should request to join a some group", async () => {
    ...
    expect(post).toHaveBeenCalledWith(`/parties/v1/parties/${currentGroupID}/request`, "glz", {
...
test("should decline a request to join a some group", async () => {
    ...
    expect(post).toHaveBeenCalledWith(`/parties/v1/parties/${currentGroupID}/request/party_id/decline`, "glz");
...
test("should join a group", async () => {
    ...
    expect(post).toHaveBeenCalledWith(`/parties/v1/players/${currentUserId}/joinparty/${anotherIdGroup}`, "glz");
...
test("should leave a group", async () => {
    ...
    expect(post).toHaveBeenCalledWith(`/parties/v1/players/${currentUserId}/leaveparty/${anotherIdGroup}`, "glz");
...
test("should return all available game modes", async () => {
    fetch.mockReset();
    fetch.mockResolvedValueOnce(mockedCurrentAvailableGameModes);

    const data = await group.currentAvailableGameModes();

    expect(fetch).toHaveBeenCalledTimes(1);

    expect(fetch).toHaveBeenCalledWith(`/parties/v1/parties/customgameconfigs`, "glz");
